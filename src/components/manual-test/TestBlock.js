...
const TestBlock = inject('rootStore')(observer(
  class TestBlock extends React.Component {
    ...
    onLogVersion = async () => {
      ...
      const resp = await connectStore.getVersion(null, 'ledger-get-version-reply');
    ...
    onLogExtendedPublicKey = async () => {
      ...
      const resp = await connectStore.getExtendedPublicKey(
        null,
        'ledger-get-extended-public-key-reply',
        rootPathH
      );
      console.log(`onLogExtendedPublicKey: ${JSON.stringify(resp, null, 2)}`);
    }
    ...
    onLogSignTransaction = async () => {
      ...
      const resp = await connectStore.signTransaction(
        null,
        'ledger-sign-transaction-reply',
        inputs,
        outputs
      );
      console.log(`onLogSignTransaction: ${JSON.stringify(resp, null, 2)}`);
    }

    onLogDeriveAddress = async () => {
      const { connectStore } = this.props.rootStore;
      const resp = await connectStore.deriveAddress(
        null,
        'ledger-derive-address-reply',
        CUtils.str_to_path("44'/1815'/0'/1/0")
      );
      console.log(`onLogDeriveAddress: ${JSON.stringify(resp, null, 2)}`);
    }

    onLogShowAddress = async () => {
      const { connectStore } = this.props.rootStore;
      const resp = await connectStore.showAddress(
        null,
